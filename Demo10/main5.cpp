...
public:
  ...
  bool Evaluate(double const* const* parameters,
    ...
    // 下边是学习了官方教程后的修改，http://www.ceres-solver.org/analytical_derivatives.html
    // 得到的结果是相同的。
    if(jacobians && jacobians[0]) {
      jacobians[0][0] = -_x*exp(parameters[0][0]*_x + parameters[1][0]);
      jacobians[0][1] = -exp(parameters[0][0]*_x + parameters[1][0]);
    }    
...
int main(int argc, char **argv) 
{
  ...
  //生成随机数：用的库函数，而不是opencv。
  default_random_engine generator;
  normal_distribution<double> distribution(0.0,0.5);//均值和方差,这里方差为0.5还是挺大的。
  //y = exp(0.3*x + 0.1)
